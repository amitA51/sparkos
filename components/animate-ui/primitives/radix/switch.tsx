import * as React from "react"
import * as SwitchPrimitives from "@radix-ui/react-switch"
import { motion, MotionProps } from "framer-motion"
import { cn } from "@/lib/utils"

const Switch = React.forwardRef<
    React.ElementRef<typeof SwitchPrimitives.Root>,
    React.ComponentPropsWithoutRef<typeof SwitchPrimitives.Root>
>(({ className, ...props }, ref) => (
    <SwitchPrimitives.Root
        className={cn(
            "peer inline-flex h-[24px] w-[44px] shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50",
            className
        )}
        {...props}
        ref={ref}
    />
))
Switch.displayName = SwitchPrimitives.Root.displayName

interface SwitchThumbProps extends React.ComponentPropsWithoutRef<typeof SwitchPrimitives.Thumb> {
    pressedAnimation?: MotionProps['whileTap']
}

const SwitchThumb = React.forwardRef<
    React.ElementRef<typeof SwitchPrimitives.Thumb>,
    SwitchThumbProps
>(({ className, pressedAnimation, ...props }, ref) => (
    <SwitchPrimitives.Thumb
        asChild
        {...props}
        ref={ref}
    >
        <motion.span
            className={cn(
                "pointer-events-auto block h-5 w-5 rounded-full bg-background shadow-lg ring-0",
                className
            )}
            whileTap={pressedAnimation}
        />
    </SwitchPrimitives.Thumb>
))
SwitchThumb.displayName = SwitchPrimitives.Thumb.displayName

export { Switch, SwitchThumb }
