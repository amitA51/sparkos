    ...
    .onRun(async () => {
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            try {
                ...
                const isEnabled = notificationsSettings.habitRemindersEnabled ?? settings.enableHabitReminders;

                if (isEnabled === false) continue; // Explicitly checked for false/undefined

                // Helper to check quiet hours with new structure support
                const checkQuietHours = (s: any) => {
                    const ns = s.notificationsSettings || {};
                    const enabled = ns.quietHoursEnabled ?? s.quietHoursEnabled;
                    if (!enabled) return false;

                    const start = ns.quietHoursStart ?? s.quietHoursStart ?? '22:00';
                    const end = ns.quietHoursEnd ?? s.quietHoursEnd ?? '08:00';
                ...
                // Check quiet hours
                if (checkQuietHours(settings)) continue;
 ...
 */
export const cleanupHabitNotificationFlags = functions.pubsub
